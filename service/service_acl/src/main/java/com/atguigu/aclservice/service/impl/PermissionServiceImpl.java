...
//        });
    ...
    public static List<Permission> buildPermission(List<Permission> permissionList) {
        ...
        //把所有菜单数据进行遍历,得到顶层菜单pid=0,设置level = 1
    ...
    private static Permission selectChildren(Permission permissionNode, List<Permission> permissionList) {
        //1.因为向一层菜单里面放二层菜单,二层里面放三层.. 把对象初始化
        permissionNode.setChildren(new ArrayList<>());

        //2.遍历所有菜单的list集合,判断id和pid的值是否相同
    ...
    private void selectPermissionChildById(String id, List<String> idList) {
        ...
        //把childIdList里面菜单的id值获取出来,封装到idList中去,并且做递归查询
